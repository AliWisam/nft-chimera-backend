...
let refinedTags = typeof req.body.tag === "string" ? JSON.parse(req.body.tag) : req.body.tag

console.log("mime type---------", req.file.mimetype.toLowerCase().split('/').join(','))

console.log('file----', req.file)
    const image = new Image({
      image : "http://localhost:5000/uploads/"+req.file.originalname.toLowerCase().split(' ').join('-'),
...
const getImageById = async (req,res)=>{
  try {
    ...
    if(image.length < 1) return res.status(400).send('the image with given tokenId is not found');
...
const changeOwner =  async (req , res)=>{
  try { 
    var image = await Image.findOneAndUpdate({tokenId : req.params.id}, {Owner : req.body.owner}, {new : true });
...
module.exports = { uploadFile, downloadFiles, getFilesList, changeOwner, deleteImage , getImageById, addTag};